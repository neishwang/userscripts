...
| [`twitch-card-options-everywhere`](scripts/twitch-card-options-everywhere.user.js) | Adds the "more options for this channel" button to every Twitch stream card, including the directory pages where Twitch omits it. "Not interested" works out of the box; if Twitch rotates its persisted-query hash, the script relearns it by watching the native menu. | [Install](https://raw.githubusercontent.com/neishwang/userscripts/main/scripts/twitch-card-options-everywhere.user.js) |
